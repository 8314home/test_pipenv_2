...
def neighbour_check(u, v, q, rows, cols, flag, visited_matrix, ans, destination):
    ...
    if is_valid(Pair(u, v), rows, cols) and visited_matrix[u][v] is False:
        print(f"checking against point {u}, {v} visited_matrix = {visited_matrix[u][v]} flag = {flag}")
        if flag is False:
            flag = True
            ans += 1
...
if __name__ == "__main__":
    ...
    matrix_2d = [[0]*6]*6  # this kind of operation will create copies of reference as list is an object, any update will update all columns
